...
export const CreativeCity = () => {
    ...
    for (let x = -gridSize; x <= gridSize; x++) {
      for (let z = -gridSize; z <= gridSize; z++) {
        ...
        // Leave center open for generic lobby
  ...
  return (
    <group>
      ...
      {/* Buildings - No Physics in Creative Mode for "Ghost" effect? 
         User said "Passable buildings". So just Mesh, no Physics useBox. */}
      {buildings.map((b, i) => (
        <group key={i} position={b.position}>
          <mesh castShadow receiveShadow>
            <boxGeometry args={b.size} />
            <meshStandardMaterial color={b.color} transparent opacity={0.6} />
          </mesh>
