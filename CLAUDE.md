...
**모든 모델 사이에 비행기가 날 수 있는 공간이 반드시 있어야 한다.**

### 왜 중요한가
빌딩 그리드(GRID=7×7, SPACING=42) + PLAYER_RADIUS=2 때문에, 데코 오브젝트를 그리드 인근에 놓으면 충돌 박스가 인접 빌딩 충돌 박스와 겹쳐 비행 불가 구역이 생긴다.

### 빌딩 그리드 범위
- 각 타운 기준 x, z 방향으로 최대 `±(3.5 × 42 + 5 + 13) = ±165` 유닛
- z/x 방향 절대 안전 구역: 타운 중심에서 `|z| > 130` 또는 `|x| > 170` (빌딩이 절대 존재하지 않는 영역)

### 규칙

1. **물리 충돌(registerBuilding)은 제일 얇은 부분 기준으로 잡아라** — 이건 비주얼 모델 얘기가 아니라, 비행기가 튕기는 그 물리법칙(AABB 충돌 박스) 얘기다. 단순 직육면체 하나로 표현할 때는 모델의 제일 두꺼운 부분(날개, 돌출부 등)이 아니라 **제일 얇은 core 부분(기둥·몸체 shaft)** 기준으로 w/d를 설정한다. 돌출된 부분은 물리 없이 통과 가능해도 괜찮다. 박스를 여러 개 쓸 여유가 되면 세밀하게 표현해도 되지만, 단순하게 할 거면 반드시 얇게 잡는 것이 원칙.
2. **그리드 위에 올리지 말 것** — 데코 오브젝트 중심이 빌딩 그리드 셀 중심(zone.cx + (col-3.5)×42 ±5)에서 20유닛 이내면 안 됨.
3. **그리드 밖에 배치하면 안전** — `|z from zone center| > 130` 또는 `|x from zone center| > 170` 위치는 어떤 x/z 위치든 빌딩과 겹치지 않음.
4. **비행 공간 최소 15유닛** — 인접한 모든 충돌 박스 사이 빈 공간이 15유닛 이상 보장되어야 한다 (플레이어 폭 PLAYER_RADIUS×2=4 기준, 여유 11유닛).
